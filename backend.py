...
def process_video():
    try:
        ...
        # Download the video
        print("YOUTUBE: Download started")
        download_video(youtube_url, source)
        
        # Process video and audio asynchronously
        workflow = VideoProcessingWorkflow(video_path, output_dir)
        asyncio.run(workflow.split_video_and_audio())
        
        # # Create YOLO chunks and push embeddings
        # create_yolo_chunks()  # This also pushes embeddings related to YOLO
        # push_embeddings()  # This pushes other embeddings except YOLO
        
        # Send success response after everything is done
        return jsonify({"message": "Video processing completed successfully"}), 200
